...
- [ ] 情境符箓 hover/focus 后放大幅度足够阅读文字；允许临时遮蔽香炉、线香或其他符箓。
- [ ] 预防符箓 hover/focus 后放大幅度足够阅读文字；允许临时遮蔽香炉、线香、桌面边界或其他符箓。
...
- [ ] 从左侧第一炷开始燃烧，续香后依次推进。
- [ ] 当前线香进度由倒计时驱动，不另起计时器。
- [ ] 主祭台 `smoke.png` 仍作为核心烟雾图层；`near` / `far` 只作为轻量 CSS 烟缕锚点，不新增素材插槽。
- [ ] 主祭台烧香中烟雾在正常观看距离下可辨认，1-3 套布局下不会连成大片或遮挡相邻香炉。
- [ ] 休息中当前线香保持已烧完状态，不继续表现为燃烧。
- [ ] 休息中只保留短暂余烟；完成闭盖后隐藏线香，不残留持续烟雾。
- [ ] 系统开启“减少动态效果”时，主祭台烟雾降级为弱化静态提示或短促淡出。
- [ ] 小窗 `compact` 烧香中必须有克制烟雾动画；优先复用 `smoke.png` 图层和轻量 CSS，不新增素材族、不引入高负载粒子。
...
- [ ] Web 版主祭台 1 / 2 / 3 套布局。
- [ ] Web 版主祭台 `resting` 余烟、`completed` 无持续烟雾和 `prefers-reduced-motion` 降级。
- [ ] Tauri 开发版原生 WebView 中，主祭台烧香烟雾可辨认，完成闭盖后无持续烟雾残留。
