...
小窗中用户创建几项任务，就显示几个 Q 版香炉槽位，最多 3 个。主祭台和小窗共享业务状态，但素材可以分为 `stage` 和 `compact` 两套。点击“开始创造”先进入完整窗口模式，展示主祭台和符箓；点击“收起到小窗”、关闭确认中选择“保留并收起”，或桌面端进入新倒计时段后，才切回小窗模式。小窗香炉只承担低干扰计时器和返回完整窗口的职责，点击任意香炉都只用于展开并聚焦完整窗口，不直接开始或续香。当前桌面端把 `390px × 620px` 的透明无标题栏窗口作为小窗模式，把 `960px × 760px` 的标准标题栏窗口作为完整窗口模式。
...
- 应用图标视觉基准为米纸底、淡墨山水、月形和亮金聚焦符箓线稿；符箓是小尺寸下的主识别点，避免大红底、高光香炉、火焰闪光和厚重拟物。
- 页面级视觉基准见 `docs/APP_VISUAL_STYLE_GUIDE.md`；Tauri 完整窗口、自用 `.app` 和 Web 开发入口都应保持米纸、淡墨青灰和亮金主识别色一致。
