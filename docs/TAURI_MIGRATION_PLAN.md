...
小窗中用户创建几套执行意图，就显示几个 Q 版香炉槽位，最多 3 个。主祭台和小窗共享业务状态，但素材可以分为 `stage` 和 `compact` 两套。小窗香炉未开始且可启动时进入同一套开始确认流程；烧香中、休息中、已完成或暂不可启动时，点击香炉用于展开并聚焦完整窗口。当前桌面端把 `390px × 620px` 作为小窗模式，把 `960px × 760px` 作为完整窗口模式；“保留并收起”表示从完整窗口切回小窗模式。
