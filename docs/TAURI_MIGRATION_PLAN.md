...
- 已接入透明窗口外壳；选择“保留并收起”进入小窗时关闭原生标题栏和窗口阴影，展开完整窗口时恢复。macOS 透明窗口依赖 `macOSPrivateApi` 配置和 `macos-private-api` Cargo feature，当前只用于内部工具验证。
...
小窗中用户创建几套执行意图，就显示几个 Q 版香炉槽位，最多 3 个。主祭台和小窗共享业务状态，但素材可以分为 `stage` 和 `compact` 两套。点击“进入仪式台”先进入完整窗口模式，展示主祭台和符箓；只有用户显式选择“保留并收起”时才切回小窗模式。小窗香炉只承担低干扰计时器和返回完整窗口的职责，点击任意香炉都只用于展开并聚焦完整窗口，不直接开始或续香。当前桌面端把 `390px × 620px` 的透明无标题栏窗口作为小窗模式，把 `960px × 760px` 的标准标题栏窗口作为完整窗口模式。
...
桌面组合回归、正式应用图标 v1、菜单栏 template icon v1 和自用安装恢复准备已经完成。后续开发顺序以 `docs/MVP_SCOPE.md` 的“后续开发优先级”为准：

1. 主祭台视觉审计、正式钟声和小窗视觉拖拽第一轮优化已完成。
2. 情境符箓分层燃烧和主祭台线香烟雾增强已完成；下一步固定走通 Demo 展示路径视觉回归。
3. 完成固定 Demo 展示路径回归后，再根据截图继续微调主祭台和小窗。
4. Demo 展示完成后，再评估登录后自动启动、更完整后台策略和全局快捷键。
5. 最后进入 DMG、签名、公证和正式发布准备。

通知横幅旧红色圆形占位图问题继续保持最低优先级，不作为近期 Tauri 迁移任务。
