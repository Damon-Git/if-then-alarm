...
小窗中用户创建几套执行意图，就显示几个 Q 版香炉槽位，最多 3 个。主祭台和小窗共享业务状态，但素材可以分为 `stage` 和 `compact` 两套。点击“进入仪式台”先进入完整窗口模式，展示主祭台和符箓；只有用户显式选择“保留并收起”时才切回小窗模式。小窗香炉未开始且可启动时进入同一套开始确认流程；烧香中、休息中、已完成或暂不可启动时，点击香炉用于展开并聚焦完整窗口。当前桌面端把 `390px × 620px` 的透明无标题栏窗口作为小窗模式，把 `960px × 760px` 的标准标题栏窗口作为完整窗口模式。
