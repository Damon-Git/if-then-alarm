...
- 已接入菜单栏“令”入口，用于显示或隐藏主窗口，不负责切换小窗/完整窗口尺寸。
- 已接入可选窗口置顶设置。
...
- macOS 菜单栏右侧状态区当前使用文字“令”作为临时入口；默认应用图标到 tray icon 的回退和无素材可处理的 `.icon_as_template(true)` 已移除。
- 正式 menubar icon v1 需要补充透明单色 template icon 素材后再接入，不复用彩色应用图标或仪式台分层 PNG。
