...
- `src-tauri/src/main.rs`：启动默认 Tauri 应用，并注册 notification plugin。
...
先按 `docs/TAURI_DESKTOP_CHECKLIST.md` 跑通最小桌面小窗验收，确认窗口尺寸、滚动、弹窗遮罩、小窗香炉舞台、窗口收起和系统通知表现稳定。通过后再考虑菜单栏入口和持久化 adapter 替换。
