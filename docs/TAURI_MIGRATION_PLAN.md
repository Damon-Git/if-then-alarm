...
当前 `PersistenceAdapter` 是同步接口。后续迁移到 Tauri 文件或 Store 时，优先采用“启动时异步加载到内存 cache，业务层继续同步读写 adapter，再异步落盘”的过渡方案，避免一次性把所有 storage 调用改成异步。
...
当前 UI 刷新使用 `window.setInterval`，真实剩余时间由 `startedAt` 和 `durationSeconds` 推导。系统通知已经通过 `src/lib/notificationAdapter.ts` 接入，并在每段专注/休息开始时设置延迟提醒。Tauri 阶段后续还需要补：
...
4. 接入菜单栏入口，用于显示/隐藏主窗口。
5. 最后再做置顶、隐藏 Dock、全局快捷键和更完整的通知交互。
...
- `src-tauri/icons/icon.png` 仍是临时占位应用图标，只用于通过 Tauri 编译和开发验证。
- macOS 菜单栏右侧状态区当前使用文字“令”作为临时入口，只用于验证窗口显示/隐藏。
- 应用图标、菜单栏 template icon 和通知图标未来应分开设计、分开验收。
- 当前阶段不制作正式图标，不生成 macOS iconset，不接真实打包发布流程。
...
- 迁移初期不应立即删除旧 `localStorage` 数据，应保留回滚路径。
...
- `src-tauri/src/main.rs`：启动默认 Tauri 应用，注册 notification plugin，并创建菜单栏入口。
- `src-tauri/capabilities/default.json`：为主窗口启用默认核心权限、窗口关闭/收起权限和通知权限，后续文件等权限按能力逐项添加。
...
桌面运行需要本机已安装 Rust 工具链。当前前端已调用窗口和通知相关 Tauri API，历史、session 和设置仍走 Web localStorage adapter。
...
先按 `docs/TAURI_DESKTOP_CHECKLIST.md` 跑通最小桌面小窗验收，确认窗口尺寸、滚动、弹窗遮罩、小窗香炉舞台、窗口收起、菜单栏入口和系统通知表现稳定。通过后再考虑持久化 adapter 替换。
