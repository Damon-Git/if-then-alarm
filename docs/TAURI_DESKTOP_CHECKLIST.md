# Tauri 桌面验收清单

本清单用于每次修改 Tauri 外壳、小窗布局、弹窗、滚动、通知或桌面相关行为后做手动验收。当前阶段只验收内部 `.app` 冒烟，不验收 DMG、签名、公证或正式发布。

桌面行为的组合回归顺序见 `docs/DESKTOP_BEHAVIOR_REGRESSION.md`。

## 验收前准备

- [ ] 当前工作区没有未保存的重要改动。
- [ ] 执行 `npm run check:desktop-config` 通过。
- [ ] 执行 `npm run test` 通过。
- [ ] 执行 `npm run build` 通过。
- [ ] 执行 `cargo check` 通过。
- [ ] 如果 `5173` 被其他应用占用，先释放端口；如果是本项目 Vite 服务，可直接复用。
- [ ] 修改窗口外壳、尺寸切换或小窗拖拽后，确认没有正在运行的 debug 或 release 进程，再执行 `npm run check:tauri-window-roundtrip` 通过。
- [ ] 原生往返冒烟结束后，确认 ignored `artifacts/tauri-window-roundtrip/` 中只包含应用窗口截图和审计记录，实际桌面 JSON 逐字节不变。
- [ ] 修改桌面持久化或准备长期自用基线时，确认没有正在运行的 debug 或 release 进程，再执行 `npm run check:tauri-persistence-recovery` 通过。
- [ ] 原生损坏恢复冒烟结束后，确认 ignored `artifacts/tauri-persistence-recovery/` 中只包含应用窗口截图和审计记录，实际桌面 JSON 逐字节不变。

## 内部打包冒烟

内部打包说明见 `docs/MACOS_INTERNAL_BUILD.md`。

- [ ] 执行 `npm run tauri:build` 通过。
- [ ] 生成 `src-tauri/target/release/bundle/macos/急急如律令.app`。
- [ ] `.app/Contents/MacOS/jiji-rululing` 存在。
- [ ] `.app/Contents/Resources/急急如律令.icns` 存在。
- [ ] `.app/Contents/Info.plist` 中 `CFBundleIdentifier` 为 `com.damon.jijirululing`。
- [ ] `.app/Contents/Info.plist` 中版本为 `0.2.0`。
- [ ] 双击或右键打开 `.app` 后能启动窗口。
- [ ] 打包版初始窗口接近 `960px × 760px`。
- [ ] Dock 和 `.app` 显示 `app-icon-v1.png` 对应的新应用图标。
- [ ] 打包版菜单栏右侧能看到简化香炉 template icon。
- [ ] 打包版菜单栏 template icon 没有复用彩色应用图标。
- [ ] 打包版保存复盘后重新打开，历史记录仍可查看。
- [ ] 当前只验收内部 `.app`，不验收 DMG、签名、公证或正式发布。

## 启动桌面窗口

```bash
npm run tauri:dev
```

- [ ] 命令能启动 Tauri 桌面窗口。
- [ ] 窗口标题显示“急急如律令”。
- [ ] 初始窗口接近 `960px × 760px`。
- [ ] 页面内容加载的是本项目，而不是其他占用 `5173` 的页面。
- [ ] 首次编译后，再次启动速度明显变快。

## 小窗基础布局

- [ ] 小窗下没有横向滚动条。
- [ ] 应用内标题区不挤占过多第一屏高度。
- [ ] 填写页、复盘页和完整状态下，“历史”和“设置”按钮可见且可点击。
- [ ] 小窗 ritual 阶段隐藏“历史”和“设置”按钮。
- [ ] 当前计时模式显示清楚，但不喧宾夺主。
- [ ] 页面主体可以纵向滚动，滚动时没有内容被系统标题栏遮住。

## 窗口行为

- [ ] 小窗模式尺寸接近 `390px × 620px`。
- [ ] 完整窗口模式尺寸接近 `960px × 760px`。
- [ ] 手动把完整窗口拖窄后，仍显示完整仪式台，不自动切换为小窗香炉舞台。
- [ ] Tauri 桌面环境下，设置面板显示“窗口置顶”开关。
- [ ] Tauri 桌面环境下，设置面板显示“在 Dock 中显示”开关。
- [ ] 浏览器环境下，设置面板不显示“窗口置顶”和“在 Dock 中显示”开关。
- [ ] 开启“窗口置顶”后，当前窗口保持在其他窗口上方。
- [ ] 关闭“窗口置顶”后，当前窗口恢复普通层级。
- [ ] 重启后，“窗口置顶”开关保留上次设置。
- [ ] 默认情况下，“在 Dock 中显示”处于开启状态。
- [ ] 关闭“在 Dock 中显示”后，Dock 中不再显示应用图标。
- [ ] Dock 图标隐藏后，仍可通过菜单栏 template icon 入口重新打开窗口。
- [ ] 重新开启“在 Dock 中显示”后，Dock 中恢复显示应用图标。
- [ ] 重启后，“在 Dock 中显示”开关保留上次设置。

## 菜单栏入口

- [ ] macOS 菜单栏右侧状态区能看到简化香炉 template icon。
- [ ] 深色和浅色菜单栏下，template icon 都清晰可读。
- [ ] 点击右侧 template icon，当前窗口会隐藏。
- [ ] 再次点击右侧 template icon，当前窗口会按隐藏前的尺寸模式重新显示并获得焦点。
- [ ] 菜单栏 template icon 入口不负责切换小窗/完整窗口尺寸，只负责显示或隐藏。
- [ ] 通过菜单栏入口隐藏窗口后，当前计时和系统通知不受影响。

2026-05-31 正式 menubar icon v1 已接入：Rust 嵌入 `menubar-icon-v1@2x.png`，设置 `.icon(...)` 和 `.icon_as_template(true)`，并移除临时 `.title("令")`。需要在 release bundle 中逐项复验本节交互行为。

2026-06-01 已完成 release bundle 人工验收：菜单栏不再显示文字“令”；深色和浅色菜单栏下图标均清晰可读；点击图标可以隐藏窗口，再次点击可以恢复窗口并聚焦；菜单栏点击不会改变小窗或完整窗口尺寸模式。

## 图标资产

- [ ] `src-tauri/icons/app-icon/app-icon-v1.png` 是 `1024px × 1024px` RGBA 应用图标 v1 源图。
- [ ] `src-tauri/icons/app-icon/app-icon-v1@2x.png` 与源图内容一致，用于 Tauri Retina ICNS 打包。
- [ ] 应用图标保持米纸底、淡墨山水、月形和亮金聚焦符箓线稿的视觉基准；`64px` 和 `160px` 预览中符箓仍是一眼可见的主识别点。
- [ ] `bundle.icon` 只接入 `icons/app-icon/app-icon-v1@2x.png`。
- [ ] `src-tauri/icons/app-icon/placeholder-icon.png` 仍保留，但不再接入 bundle。
- [ ] 应用图标、菜单栏图标和通知图标的职责已在 `src-tauri/icons/README.md` 和各子目录 README 中分开记录。
- [ ] `src-tauri/icons/menubar-icon/menubar-icon-v1.png` 是 `16px × 16px` RGBA 透明单色素材。
- [ ] `src-tauri/icons/menubar-icon/menubar-icon-v1@2x.png` 是同轮廓的 `32px × 32px` RGBA Retina 素材。
- [ ] 当前 tray 使用独立的菜单栏 template icon，不回退到默认应用图标，也不复用应用图标 v1。

## 页面视觉风格

- [ ] 完整窗口页面保持 `docs/APP_VISUAL_STYLE_GUIDE.md` 中的米纸暖白、淡墨青灰和亮金主识别色。
- [ ] 主按钮、选中态和当前顶部入口使用亮金；普通次级按钮不再使用旧朱红作为品牌强调。
- [ ] 页面背景不出现大面积圆团、光斑、散景、强渐变球或与内容无关的装饰。
- [ ] 桌面完整窗口和移动宽度下，填写、设置、历史、复盘和弹窗共享同一套面板、边框、按钮和焦点态。

## 填写页

- [ ] 默认显示 1 项任务。
- [ ] 可以创建到 3 项任务。
- [ ] 长目标性执行意图输入时，输入框和页面滚动正常。
- [ ] 校验错误显示后不会撑出横向滚动。
- [ ] 点击“开始创造”后能打开仪式台。
- [ ] 点击“开始创造”后先打开完整窗口和主祭台，不直接进入纯香炉小窗。

## 小窗仪式台

- [ ] 创建 1 套时显示 1 个香炉。
- [ ] 创建 2 套时显示 2 个香炉。
- [ ] 创建 3 套时显示 3 个香炉。
- [ ] 小窗下显示香炉舞台，不显示完整主祭台三栏布局。
- [ ] Tauri ritual 小窗没有原生标题栏、红黄绿窗口按钮或默认白色窗口底板。
- [ ] 小窗香炉显示 `censer/compact` 正式 PNG 素材，不退回纯 CSS 占位图。
- [ ] `body/lid/mouth/ash/feet` 图层都能正常叠放，没有散开或明显错位。
- [ ] 创建 1 / 2 / 3 套时，香炉数量分别为 1 / 2 / 3 个，且始终单行并排。
- [ ] 香炉横排上方存在自然、足够大的独立透明拖拽热区。
- [ ] 从拖拽热区开始拖动时，小窗可以可靠移动，并且不会展开完整窗口。
- [ ] 从香炉炉身或线香区域按下后明显移动时，小窗会跟随鼠标移动，并且不会在松手后误触展开；正常单击仍可展开完整窗口。
- [ ] 小窗拖动到新位置后，点击香炉展开完整窗口，再“保留并收起”时，小窗回到上一次移动后的位置。
- [ ] 香炉内线香根数与所选香数一致。
- [ ] 每套选择 1 / 2 / 3 炷香时，线香数量分别为 1 / 2 / 3 根。
- [ ] 线香不会顶出香炉槽位或背景框。
- [ ] 小窗烧香中当前香炉有克制烟雾动画，休息中烟雾收束或隐藏，完成态不残留持续烟雾。
- [ ] 小窗烧香中 hover/focus 当前香炉会完整显示剩余时间；浮层位于香炉按钮内部区域、不被 WKWebView 或透明窗口内容区裁剪；移开/失焦后不常驻显示倒计时。
- [ ] 点击未开始香炉只尝试展开完整窗口，不打开开始确认弹窗。
- [ ] 单击任意香炉仍可以展开完整窗口；按住炉身或线香区域明显拖动时则移动小窗，不展开完整窗口。
- [ ] 小窗下烧香中、休息中或已完成的香炉点击后展开并聚焦完整窗口。
- [ ] 展开完整窗口后恢复原生标题栏和窗口阴影。
- [ ] 展开完整窗口后，原生外框重新接近 `960px × 760px`，不会累计标题栏高度。
- [ ] 小窗下已有活动任务时，点击其他未开始香炉不会启动新任务，只展开完整窗口。
- [ ] 小窗 ritual 阶段不常驻显示状态文案、倒计时和长执行意图摘要。
- [ ] 小窗 ritual 阶段的香炉按钮仍通过 aria-label 保留当前状态、香数和倒计时。

2026-06-02 已人工验证本轮小窗交互和视觉微调：正常单击香炉仍展开完整窗口；从炉身或线香区域按住拖动时，小窗会跟随鼠标移动，松手后不误触展开；两个和三个香炉时的横排间距收紧为 `6px` 后观感通过；小窗弱状态香炉提高不透明度后观感通过。

## 弹窗

- [ ] 单炷结束弹窗按钮显示完整。
- [ ] 休息结束弹窗按钮显示完整。
- [ ] 放弃本轮确认弹窗显示完整。
- [ ] 历史删除或清空确认弹窗显示完整。
- [ ] 弹窗打开时，背景内容不会横向溢出。

## 计时流程

- [ ] 点击开始后，倒计时正常刷新。
- [ ] 单炷结束后弹出休息或续香选择。
- [ ] 选择休息后进入休息倒计时。
- [ ] 休息结束后可以继续下一炷香。
- [ ] 多炷香会按第 1、2、3 炷依次推进。
- [ ] 所有香完成后停留完成场景，用户点击“进入复盘”后进入复盘页。

## 系统通知

- [ ] 首次开始计时时，macOS 会请求通知权限，允许后继续当前流程。
- [ ] 一炷香计时结束时，系统通知显示“这一炷香已经烧完。”。
- [ ] 休息计时结束时，系统通知显示“休息结束，是否继续下一炷香？”。
- [ ] 开始下一段计时后，不会继续保留上一段的待触发通知。
- [ ] 放弃本轮、保存复盘或丢弃恢复轮次后，不会继续触发旧通知。
- [ ] 进行中轮次点击“保留并收起”后，如果当前段计时结束，系统仍能发出通知。
- [ ] 系统通知中的应用识别表现单独验收；不要把菜单栏 template icon 当作通知图标。

2026-05-31 阻塞记录：从 `/Users/damon/Applications/急急如律令.app` 明确启动新 bundle 并触发 10 秒开发模式通知后，通知仍显示旧红色圆形占位图。安装路径和工作区 bundle 的 ICNS 已一致为 `1024px × 1024px` 应用图标 v1；LaunchServices 仍索引包含旧 `128px × 128px` 占位 ICNS 的 `/Users/damon/Desktop/急急如律令.app`。当前按 LaunchServices 或通知中心缓存问题处理，通知应用识别图标 v1 不标记为通过。

2026-06-01 只读复查：`/Users/damon/Applications/急急如律令.app` 和 `/Applications/急急如律令.app` 当时均不存在；LaunchServices 当时同时索引 Desktop 旧占位副本、工作区 release bundle 和废纸篓中的正式图标 v1 副本。当时最强证据是 Desktop 旧副本的 LaunchServices 索引，通知中心缓存也不能排除。详细哈希与安全处理边界见 `docs/MACOS_INTERNAL_BUILD.md`。该次复查未将通知应用识别图标 v1 标记为通过。

2026-06-01 局部处理：用户删除 Desktop 旧副本后，已局部注销废纸篓中的旧占位副本，并强制注册工作区 release bundle。等待复查后，LaunchServices 中 `com.damon.jijirululing` 只剩工作区 release bundle；应用也已从该精确路径启动。仍需人工触发下一条通知，确认通知中心是否已经展示正式图标 v1。通知应用识别图标 v1 暂不标记为通过。

2026-06-01 通知实测：从工作区 release bundle 精确路径启动应用并触发 10 秒开发模式通知后，通知横幅仍显示旧红色圆形占位图。触发前后 LaunchServices 均只剩工作区 release bundle，因此当前最可能根因已收敛为通知中心侧缓存。未重启通知中心进程，未重置通知权限，未修改系统设置，未重建或清理 LaunchServices 数据库。通知应用识别图标 v1 仍不标记为通过。

2026-06-01 优先级调整：经用户授权，已通过正常 `SIGTERM` 重启当前用户的通知中心 UI agent，并从工作区 release bundle 再次触发 10 秒通知；通知横幅仍显示旧红色圆形占位图。通知正文和触发时机正常，当前版本仅用于个人自用。本问题降为最低优先级并暂缓处理，后续仅在正式发布准备或用户主动重新提升优先级时继续排查。

## 复盘与历史

- [ ] 复盘页在小窗下可以滚动到保存按钮。
- [ ] 复盘长文本不会撑出横向滚动。
- [ ] 复盘页在约 `390px` 宽度下仍突出“本次完成度”和“一句复盘”，本轮摘要和任务详情不压过主输入。
- [ ] 一句复盘为空时报错时，错误文案、错误边框和无障碍属性都绑定到“一句复盘”输入框。
- [ ] 复盘页“放弃本轮”在窄窗口下仍是次要危险操作，不抢过“保存复盘”。
- [ ] 保存后历史记录出现本轮记录。
- [ ] 重启后，历史记录仍能查看。
- [ ] 历史面板可以滚动查看多条记录。
- [ ] 历史筛选、搜索、导入、导出按钮在小窗非 ritual 阶段不拥挤到不可用。
- [ ] 点击“导出历史”会打开 macOS 保存面板。
- [ ] 取消保存面板后，不显示导出成功 toast。
- [ ] 选择保存路径后，会写入本应用历史 JSON。
- [ ] 点击“导入历史”会打开 macOS 打开文件面板。
- [ ] 取消打开文件面板后，不显示导入失败 toast。
- [ ] 选择本应用导出的 JSON 后，能完成导入并显示新增/跳过数量。
- [ ] 点击“导出完整备份”会打开 macOS 保存面板。
- [ ] 完整备份 JSON 包含历史、当前未完成轮次和设置。
- [ ] 点击“导入完整备份”会打开 macOS 打开文件面板。
- [ ] 选择完整备份后，会先弹确认，不会直接覆盖。
- [ ] 确认导入后，历史、未完成轮次和设置会被替换。
- [ ] 选择历史导出文件导入完整备份时，会显示失败提示且不覆盖当前数据。
- [ ] 设置面板在小窗非 ritual 阶段可以完整滚动查看。

## 关闭与恢复

关闭与恢复的产品语义见 `docs/TAURI_CLOSE_RESTORE_NOTES.md`。

- [ ] 填写页已有草稿时点击窗口关闭按钮，会出现应用内确认弹窗。
- [ ] 草稿关闭确认中点击“继续填写”，窗口保持打开。
- [ ] 草稿关闭确认中点击“关闭窗口”，窗口关闭，重新启动后不恢复草稿。
- [ ] 仪式台进行中点击窗口关闭按钮，会出现应用内确认弹窗。
- [ ] 进行中关闭确认里点击弹窗关闭按钮，窗口保持打开，当前轮次不变化。
- [ ] 进行中关闭确认里点击“放弃并退出”，窗口关闭，重新启动后不恢复本轮且不写入历史。
- [ ] 进行中关闭确认里点击“保留并收起”，窗口切回小窗模式且计时继续。
- [ ] 重新启动后，如果有未完成轮次，会出现恢复提示。
- [ ] 点击恢复后能回到当前轮次。
- [ ] 点击丢弃后回到填写页，不写入历史。
- [ ] 保存复盘后重新启动，不会提示恢复旧轮次。

## 持久化迁移准备

- [ ] 浏览器环境下，历史、session、设置仍走 Web `localStorage` adapter。
- [ ] Tauri 环境下，历史、session、设置会写入 app data 目录下的 `persistence.v1.json`。
- [ ] `createMemoryPersistenceAdapter` 作为 Tauri 桌面存储的同步缓存层，业务模块不直接调用异步文件 API。
- [ ] `PERSISTENCE_STORAGE_KEYS` 包含 history、current session、settings 三个 key。
- [ ] 桌面 JSON 纯函数 schema 已覆盖默认值、迁移、坏数据降级和 snapshot 映射。
- [ ] 首次运行 Tauri 桌面版时，如果没有桌面 JSON，会从旧 `localStorage` 生成第一份文件。
- [ ] 从旧 `localStorage` 迁移成功时，会显示“已迁移到桌面本地数据文件。”。
- [ ] 桌面 JSON 损坏时，会先备份为 `persistence.v1.corrupt-*.json`，再尝试恢复。
- [ ] 桌面 JSON 损坏并恢复时，会显示“本地数据文件异常，已保留备份并尝试恢复。”。
- [ ] 桌面持久化初始化失败时，会显示“桌面数据文件暂不可用，本次将临时使用浏览器存储。”。
- [ ] 桌面 JSON 写入失败时，会显示“本地数据文件写入失败，本次更改可能暂未落盘。”。
- [ ] 持久化迁移计划记录在 `docs/PERSISTENCE_MIGRATION_PLAN.md`。
- [ ] 桌面 JSON 文件规格记录在 `docs/DESKTOP_PERSISTENCE_JSON_SPEC.md`。
- [ ] 第一版桌面落盘目标是 app data 目录下的 `persistence.v1.json`。
- [ ] macOS 开发环境可在 `~/Library/Application Support/com.damon.jijirululing/persistence.v1.json` 查看桌面 JSON。
- [ ] 当前阶段不清理旧 `localStorage` 数据。

## 当前不验收

- [ ] 不验收全局快捷键。
- [ ] 不验收完整发布 iconset。
- [ ] 不验收真实 macOS 打包发布。
