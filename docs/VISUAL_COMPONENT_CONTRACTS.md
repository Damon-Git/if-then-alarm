...
- 主祭台线香不承担点击交互。
- 小窗香炉按钮的普通单击只用于展开完整窗口，不复用主祭台点击规则；从炉身或线香区域按下后明显拖动时，只移动小窗并抑制松手展开。
...
信息卡和运行态 `.timer-panel` 的锚点必须落在背景空白区，彼此之间也不能重叠。单槽、双槽和三槽布局可以使用不同水平偏移，但不能覆盖线香、香炉主体或预防符箓；调整位置时不得改变香炉自身水平基线。当前 CSS 会把首槽和三槽中间位的计时卡向右偏移，把末槽计时卡向左偏移；香炉信息卡使用相反方向，从而在 hover 时保持清晰分离。
