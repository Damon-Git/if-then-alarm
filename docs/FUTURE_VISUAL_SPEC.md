...
小窗模式未来使用克制可爱的 Q 版香炉素材。小窗 ritual 阶段只显示用户创建的 1-3 个小香炉，并且始终单行并排；不显示主祭台、符箓、历史、设置、标题、意图摘要、说明性状态文案或灰色背景面板。小窗香炉与主祭台香炉一一对应；未开始时可以作为香炉入口进入开始确认。当前实现中的小窗香炉只是槽位结构占位，不代表最终素材风格。
