...
- `idle`：情境符箓是启动入口，香炉开盖，线香完整待燃，不显示计时面板。
- `burning`：情境符箓禁用，香炉开盖，当前线香按进度燃烧，显示计时面板。
- `resting`：情境符箓禁用，香炉开盖，当前线香保持上一段已烧完状态，不表现为继续燃烧，显示休息计时面板。
- `completed`：情境符箓弱化，香炉闭盖，线香隐藏，不显示计时面板。
