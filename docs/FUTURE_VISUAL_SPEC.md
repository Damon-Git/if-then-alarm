...
当前版本先实现无复杂动画的状态落点：开始后情境符箓退场，全部线香烧完后预防性符箓退场，香炉闭盖并弱化。未来接入情境符箓燃烧动画时，只在该套第一次从 `idle` 确认进入 `burning` 时播放一次，目标时长约 2 秒，动画结束后符箓消失；同一香炉后续续香不再播放燃烧动画。
...
情境符箓燃烧动画、预防性符箓退场、香炉合盖和线香燃尽都必须由既有状态驱动，不能额外创建一套与业务状态并行的隐藏流程。动画失败或被降级时，界面仍应落到相同结果：情境符箓不可见，已完成套组的预防性符箓不可见，香炉闭盖且弱化。
