...
线香燃烧进度应由倒计时状态驱动。当前 MVP 保留香数和倒计时字段，并已接入主祭台线香临时 PNG，为未来的视觉进度提供数据来源。

线香统一从 `src/components/IncenseVisual.tsx` 接入视觉层。当前主祭台线香使用静态图片图层和进度数据，不做燃烧动画、香灰掉落、火星闪动或烟雾动画。未来要做线香燃烧、香灰、火星、烟雾时，优先替换 `IncenseVisual` 内部结构和样式，不改计时、休息、续香、复盘等业务逻辑。
