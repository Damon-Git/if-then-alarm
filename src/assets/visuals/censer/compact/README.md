...
- `mouth.png` 在 compact 组件中同时承担炉口后沿与前沿来源：原始 `mouth` 层位于香灰和线香后方，组件生成的 `mouth-front` 只显示下半弧并位于线香前方。`mouth-front` 不是新的素材插槽。
- 炉口合成顺序固定为后沿、香灰、线香、炉身、前沿。线香和短香脚应从白色香灰区域中间伸出，不能整体位于香炉后方，也不能压在棕色前沿上。
- 后续替换 `mouth.png` 时必须保持炉口椭圆、白色香灰和香炉主体共用同一水平中心；如果椭圆中线在源画布中的位置发生变化，需要同步调整前沿裁切并重跑 `npm run check:compact`。
