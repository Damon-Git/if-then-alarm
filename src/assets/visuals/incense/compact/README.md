...
本目录用于放置 macOS 小窗中的线香图层。当前已有一组临时测试 PNG，用于验证小窗线香从 CSS 占位切换为图片分层渲染。
...
当前四层由 `scripts/generate-stage-incense-assets.mjs` 按 `192px × 192px` 画布确定性生成，只用于验证素材链路和进度驱动，不代表最终小窗线香设计。小窗线香需要优先保证在窄窗口中清楚可读，不追求复杂细节。
